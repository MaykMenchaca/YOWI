...
```php
$adminId = ds_require_admin();          // exige sesión + 2FA activo
...
`$permitirSinEnrolar = true` es **solo** para los 4 endpoints que un admin necesita antes
...
- [ ] Si es un endpoint admin nuevo: ¿llama a `ds_require_admin()`? ¿el POST valida CSRF?
